...
A region satisfying both is emitted once, typed CGI_GGF+CGI_TJ.
...
def main():
    ...
    for chrom, size in zip(fa.references, fa.lengths):
        ...
        # A region called by both parameterizations is emitted once, typed
        # as both - "called by both" means OVERLAPS, not identical (s, e):
        # the two masks are built from different per-window thresholds and
        # gap-merged independently, so their region boundaries essentially
        # never coincide exactly even when they describe the same island
        # (an exact-tuple-match check here was a real bug, 2026-09-11 - it
        # reported only 17 "both" out of >1M candidate regions).
        ggf_regions = per_type_regions["CGI_GGF"]  # sorted by construction
        tj_regions = per_type_regions["CGI_TJ"]

        def overlaps_any(sorted_regions, s, e):
            for rs, re_, *_ in sorted_regions:
                if rs >= e:
                    break
                if re_ > s:
                    return True
        ...
        for s, e, length, gc_frac, obs_exp, n_cpg in ggf_regions:
            type_name = "CGI_GGF+CGI_TJ" if overlaps_any(tj_regions, s, e) else "CGI_GGF"
            rows.append((chrom, s, e, type_name, length, gc_frac, obs_exp, n_cpg))
        for s, e, length, gc_frac, obs_exp, n_cpg in tj_regions:
            if not overlaps_any(ggf_regions, s, e):
                rows.append((chrom, s, e, "CGI_TJ", length, gc_frac, obs_exp, n_cpg))
    ...
    print(f"  {n_both} called by both parameterizations, {n_ggf} GGF-only, {n_tj} TJ-only")
