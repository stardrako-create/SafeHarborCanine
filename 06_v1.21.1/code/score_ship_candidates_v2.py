# v1.21.1: coverage-aware RRBS and explicit evidence states; historical notes below retained.
#!/usr/bin/env python3
"""
...
from bw_utils import evidence_summary, track_value, build_matched_window_background, build_narrow_window_background
...
def main():
    ...
    for c in candidates:
        ...
        c["rrbs_mean"], c["rrbs_observed_fraction"], c["rrbs_observed_bp"] = evidence_summary(rrbs_mean_bw, rrbs_cov_bw, chrom, start, end)
    ...
    atac_freq_bg = build_matched_window_background(atac_freq_bw, candidate_lengths,
                                                     stat_type="max", implicit_zero=True)
    rrbs_mean_bg = build_matched_window_background(rrbs_mean_bw, candidate_lengths, coverage_bw=rrbs_cov_bw)
    ...
    for c in candidates:
        ...
        c["final_score"] = round(weighted_sum / weight_total, 4) if weight_total > 0 and all(v is not None for v in components.values()) else None
    ...
    # Missing inputs are not biological failures and never count as completed checks.
    for c in candidates:
        missing = []
        locus = (c["chrom"], c["start"], c["end"])
        if locus not in low_mappability: missing.append("mappability")
        if c["pct_repeat"] is None: missing.append("repeat_content")
        if c["max_50bp_rolling_phyloP"] is None: missing.append("conservation")
        if find_containing_interval(tad_intervals, c["chrom"], (c["start"]+c["end"])//2) is None:
            missing.append("TAD_assignment")
        if not args.external_regulatory_bed: missing.append("external_regulatory_annotation")
        if c["final_score"] is None: missing.append("score_component")
    ...
    fieldnames = ["evaluation_status", "missing_evidence", "rrbs_observed_fraction", "rrbs_observed_bp", "chrom", "start", "end", "length", "orientation", "left_gene", "right_gene",
