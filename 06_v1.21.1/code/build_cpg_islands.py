# v1.21.1: independent typed callsets with original coordinates; no dual labels.
# Uses published numeric thresholds with this project's sliding/merging implementation.
# This is supplementary sequence annotation, not a replacement for regulatory evidence.
#!/usr/bin/env python3
"""
...
def main():
    ...
    for chrom, size in zip(fa.references, fa.lengths):
        ...
        # Keep both callsets independently; overlap is NOT equality or dual qualification.
        for type_name, regions in per_type_regions.items():
            for start, end, length, gc_frac, obs_exp, n_cpg in regions:
                rows.append((chrom, start, end, type_name, length, gc_frac, obs_exp, n_cpg))
