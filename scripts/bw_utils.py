...
site, which is what caused three real bugs:
...
3. 2026-09-12 (found by Codex, a separate working session on this same
   project - see 06_v1.21.1/ - and adopted here): rrbs_mean's window
   average included unobserved bins as measured zeros, same conflation
   class as bug 1 but at the candidate/background-read level rather than
   the track-build level. evidence_summary()/track_value()'s coverage_bw
   parameter fixes it: the mean is now taken only over bases a coverage
   track confirms were actually observed, on both the candidate side and
   (via build_matched_window_background()'s own coverage_bw passthrough)
   the background side.

keep_bins_with_evidence() is unrelated to bigwig reading: see its own
docstring for the fourth bug this file fixes (0-or-1-dog "stability").
"""
import random
...
def evidence_summary(bw, coverage_bw, chrom, start, end, min_evidence=1):
    """Mean of measured base-pair values, restricted to bases a coverage
    track confirms were actually observed - the fix for rrbs_mean's window
    average silently including unobserved (implicit-zero-padded) bins as
    if they were real 0%-methylation measurements (found 2026-09-12 by
    Codex, a separate working session on this project). This is a plain
    per-base mean of measured values, NOT a per-CpG or read-depth-weighted
    estimator - callers that need that distinction should say so.
    Mismatched track chromosome lengths and read failures raise rather
    than silently returning zero. Returns (mean_or_None, observed_fraction,
    n_observed_bases)."""
    if bw.chroms(chrom) != coverage_bw.chroms(chrom):
        raise ValueError(f"Mismatched track chromosome lengths for {chrom}")
    ...
    n = int(mask.sum())
    mean = float(np.mean(v[mask], dtype=np.float64)) if n else None
    return mean, n / (end - start), n
...
def track_value(bw, chrom, start, end, stat_type="mean", implicit_zero=False, exact=True,
    ...
    zoom-level summary. coverage_bw: restrict the mean to bases a coverage
    track confirms were observed (see evidence_summary()) - for tracks like
    RRBS methylation where the dense mean track itself may zero-pad
    unobserved bases; only supported for stat_type="mean" with
    implicit_zero=False, since it answers a different question than
    either of those."""
    ...
    if coverage_bw is not None:
        if stat_type != "mean" or implicit_zero:
            raise ValueError("coverage_bw masking only supports measured means "
                              "(stat_type='mean', implicit_zero=False)")
...
def build_matched_window_background(bw, window_lengths, n_samples=3000, seed=42,
    ...
    coverage_bw/min_evidence: forwarded to track_value() so an rrbs_mean-
    style background is coverage-masked the same way its candidate-side
    value is (see evidence_summary()) - matching statistics on both sides
    of a comparison, the same principle as stat_type/implicit_zero above,
    just for this third bug class (2026-09-12, found by Codex)."""
    ...
    while len(vals) < n_samples:
        ...
        if attempts > n_samples * 100:
            raise ValueError("Insufficient observable background windows - check the "
                              "coverage track and min_evidence, not just this function")
...
def keep_bins_with_evidence(evidence_count, min_evidence=2):
    ...
    a variability (or, since 2026-09-12, mean) track and which are left as
    a genuine bigwig gap - fixes a real bug (2026-09-10) where 0-or-1-dog
    bins wrote variability 0.0, read downstream as *maximally stable* - the
    opposite of *no/insufficient evidence*: with 0 dogs, an IQR/stdev over
    an empty slice is NaN, silently nan_to_num'd to 0.0; with 1 dog, the
    IQR/stdev of a single value really is 0 but is equally uninformative
    about stability across dogs. min_evidence=2 is the minimum needed for a
    spread statistic to mean anything at all (the mean track's own gate
    uses min_evidence=1 via `denom > 1e-9`-style checks elsewhere, since a
    mean of 1 dog is still a legitimate mean, just not a legitimate
    spread)."""
