...
O=A/'CONTROL_LOCUS_REVIEW_20260922/variant_catalogue_query_20260923'
...
projection=A/'CONTROL_LOCUS_REVIEW_20260922/UU_projection/projection_review.json'
windows=[r for r in json.loads(projection.read_text())['results'] if r['status']=='eligible_exact_window_for_variant_query']
 ...
 with pysam.FastaFile('/mnt/d/Jin2024_work/uu_gsd/genome_uu_gsd.fa') as ref:
  ...
  for source in ['AutoAndXPAR.SNPs.vqsr99.vcf.gz','AutoAndXPAR.nonSNPs.filter.vcf.gz']:
   ...
   for w in windows:
    ...
    h=next(h for h in w['hits'] if h['primary'] and h['exact_sequence_verified'])
    ...
    for r in vf.fetch(alias,h['start0'],h['end0']):
     ...
     for index,alt in enumerate(r.alts or ()):
      alleles.append({'chrom':alias,'uu_chrom':h['chrom'],'pos1':r.pos,'id':r.id,'ref':r.ref,'alt':alt,'alt_index1':index+1,'AF':None if af is None else af[index],'filters':list(r.filter),'reference_verified':ok,'within_exact_window':r.start>=h['start0'] and r.start+len(r.ref)<=h['end0'],'sequence_allele':bool(alt) and set(alt.upper())<=set('ACGT')})
    signal.alarm(0)
