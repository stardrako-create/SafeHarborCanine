...
A=Path(__file__).resolve().parent;C=A/'CONTROL_LOCUS_REVIEW_20260922';Q=C/'variant_catalogue_partial_query_20260923';O=C/'partial_variant_projection_20260923'
...
with pysam.FastaFile(str(R)) as ref,pysam.FastaFile(U) as uu:
 ...
 with pysam.VariantFile(str(out)) as vf:
  for r in vf:
   ...
   assert ref.fetch(r.contig,lo,p).upper()+v['alt']+ref.fetch(r.contig,p+len(v['ref']),hi).upper()==ref.fetch(r.contig,lo,r.start).upper()+r.alts[0]+ref.fetch(r.contig,r.stop,hi).upper()
