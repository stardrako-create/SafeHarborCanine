...
import os,json,csv,subprocess,datetime,hashlib,traceback,shutil
import pysam
A=Path(__file__).resolve().parent;C=A/'CONTROL_LOCUS_REVIEW_20260922';O=C/'repeatmasker_context_20260923_v2';O.mkdir(exist_ok=False)
...
try:
 ...
 work=Path('/mnt/d/Jin2024_work/control_repeats_20260923_v2');work.mkdir(exist_ok=False);shutil.copy2(O/'controls_context.fa',work/'controls_context.fa')
 cmd=['/home/stardrako/miniforge3/envs/cactus/bin/RepeatMasker','-species','dog','-pa','2','-dir',str(work),str(work/'controls_context.fa')];save('command.json',cmd)
 ...
 for f in work.iterdir():
  if f.is_file() and f.name!='controls_context.fa':shutil.copy2(f,O/f.name)
