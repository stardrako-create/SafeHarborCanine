...
A=Path(__file__).parent/'SV_PBGV000010/alignment_chunked';O=A/'local_extension_20260922_k71';O.mkdir(exist_ok=True)
ref='/mnt/d/Jin2024_work/uu_gsd/genome_uu_gsd.fa';chrom='NC_049253.1';K=71
...
with pysam.FastaFile(ref) as f:
 seed=f.fetch(chrom,4707939-K,4707939).upper();left=f.fetch(chrom,4707939,4708939).upper();right=f.fetch(chrom,33613308,33614308).upper()
