...
A=Path(__file__).parent/'SV_PBGV000010/alignment_chunked';O=A/'junction_competition_20260922_v2';O.mkdir(exist_ok=True)
...
with pysam.AlignmentFile(str(A/'selected_markdup.bam'),'rb') as f:
 for r in f.fetch(until_eof=True):
  if r.query_name in names and not r.is_secondary and not r.is_supplementary:
   key=r.query_name+('_mate1' if r.is_read1 else '_mate2');reads[key]=r.get_forward_sequence()
