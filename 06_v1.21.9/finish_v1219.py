...
P=Path(r'D:\Biblioteca\Bioquímica 1º ano 2025-2026\2º Semestre\Vasco M. Barreto\Safe Harbor Canino');O=P/'06_v1.21.9'
assert json.loads((O/'integration_validation.json').read_text())['status']=='passed'
note='''# Safe Harbor canino para CAR-T — relatório v1.21.9
...
'''
# Ensure the displayed control counts agree with both consensus and union results.
import csv
q=list(csv.DictReader((O/'RNA_cluster_local_fragment_counts.tsv').open(),delimiter='\t'))
for mask in ['T_enriched_all_four_runs','T_enriched_any_run']:
 counts={r['target']:int(r['unique_fragment_starts']) for r in q if r['mask']==mask}
 assert counts=={'ANO2/NTF3':0,'LOC119876429/LOC119872513':0,'NPNT/TBCK':0,'CD3D_feature_control':4,'CD3E_feature_control':4,'CD247_feature_control':7,'LYZ_feature_control':0}
report=O/'RELATORIO_ATUALIZADO_2026-09-14.md';report.write_text(note,encoding='utf-8');(O/'CURRENT.md').write_text(note,encoding='utf-8')
for p in Path('.').glob('*v1219.py'):shutil.copy2(p,O/p.name)
notes=Path(r'D:\Biblioteca\Biblioteca\Labs\Vasco Barreto Lab\Notas');name='Safe Harbor CAR-T - v1.21.9 conservacao T e SV - 2026-09-14';dest=notes/(name+'.md');dest.write_text(note,encoding='utf-8')
shared=notes/'Safe Harbor CAR-T - Relatorio atualizado - 2026-09-14.md'
if shared.exists():shutil.copy2(shared,O/'previous_Obsidian_report.md')
shared.write_text(note,encoding='utf-8')
moc=notes/'MOC-SafeHarbor Canino CAR-T.md';m=moc.read_text(encoding='utf-8');anchor='# 00 · MOC — Safe Harbor Canino para CAR-T';assert anchor in m
shutil.copy2(moc,O/'previous_MOC.md');moc.write_text(m.replace(anchor,anchor+f'\n\n> [!important] Estado mais recente: v1.21.9\n> [[{name}]] — modelo publicado aplicado às três regiões; ausência de suporte local no grupo T-enriquecido; grande SV NPNT questionada, ainda não resolvida. Nenhum safe harbor validado. As entradas abaixo são checkpoints históricos.\n',1),encoding='utf-8')
shutil.copy2(P/'CURRENT.md',O/'previous_project_CURRENT.md');(P/'CURRENT.md').write_text('# Estado atual: v1.21.9\n\n[Relatório atualizado](06_v1.21.9/RELATORIO_ATUALIZADO_2026-09-14.md) · [Verificação](06_v1.21.9/integration_validation.json).\n\nConservação publicada aplicada às três regiões; revisão RNA/ATAC exploratória; SV NPNT não resolvida após evidência contrária à perda simples de uma cópia. Scoring global permanece v1.21.8. Nenhum safe harbor CAR-T validado.\n',encoding='utf-8')
r=call('mempalace_add_drawer',{'wing':'Locus Canino','room':'v1.21.9_Conservation_T_SV_Review','content':note,'source_file':str(dest),'added_by':'Codex'})
assert not r.get('error') and not r.get('result',{}).get('isError'), 'MemPalace write failed'
(O/'mempalace_receipt.json').write_text(json.dumps(r,ensure_ascii=False,indent=2),encoding='utf-8')
hashes={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in O.iterdir() if p.is_file() and p.suffix in ['.py','.tsv','.json','.wig','.mod','.npz','.md'] and p.name!='output_hashes.json'}
(O/'output_hashes.json').write_text(json.dumps(hashes,indent=2),encoding='utf-8')
assert dest.read_text(encoding='utf-8')==report.read_text(encoding='utf-8')==shared.read_text(encoding='utf-8')
print('v1.21.9 finalized; report, Obsidian, MemPalace and project checkpoint saved. Biological validation remains open.')
