...
def read_binned(bw, chrom, size, bin_size, n_bins, sample_name="?", chunk_size=1_000_000):
    if chrom not in bw.chroms():
        raise RuntimeError(f"{sample_name}: missing chromosome {chrom}; cannot substitute measured zero")
    try:
        ...
        # Retry chunk-by-chunk, but abort if a block remains unreadable.
    ...
    # Padding is an implementation detail, not additional genomic bases.
    lengths = np.minimum(bin_size, size - np.arange(n_bins) * bin_size)
    return (arr.reshape(n_bins, bin_size).sum(axis=1, dtype=np.float64) / lengths).astype(np.float32)
